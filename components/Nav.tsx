      ...
      >
        <div className="max-w-[1200px] mx-auto flex items-center justify-between h-[68px]">
          ...
          <div className="flex items-center gap-4">
            ...
            {/* CV Modal Trigger Desktop */}
            <button
              onClick={() => setIsCVOpen(true)}
              className="hidden md:inline-flex items-center gap-1.5 bg-white/5 border border-white/15 text-white px-3.5 py-1.5 rounded-md text-xs font-mono tracking-wider hover:bg-white/10 transition-all"
            >
            ...
            <button
              ...
              className="hidden md:block bg-transparent border border-brand-secondary/50 text-brand-secondary px-5 py-1.5 rounded-md text-xs font-mono tracking-widest transition-all hover:bg-brand-secondary/15 hover:border-brand-secondary"
            >
        ...
        <AnimatePresence>
          {open && (
            <motion.div
              ...
              className="md:hidden fixed top-[68px] left-0 right-0 bottom-0 bg-brand-dark/95 backdrop-blur-xl flex flex-col items-center justify-center gap-6 z-[99]"
            >
              {links.map((l) => (
                <button
                  key={l.id}
              ...
              <button
                ...
                className="mt-2 inline-flex items-center gap-2 bg-white/10 text-white px-6 py-2.5 rounded-md text-sm font-mono tracking-widest"
              >
                <FileText size={16} className="text-brand-secondary" />
                <span>{lang === 'es' ? 'VER CV OFICIAL' : 'VIEW OFFICIAL CV'}</span>
              </button>
              <button
                onClick={() => scrollTo('contact')}
                className="bg-brand-secondary text-brand-dark px-8 py-3 rounded-md text-sm font-mono tracking-widest font-bold"
              >
                {lang === 'es' ? 'CONTRÁTAME' : 'HIRE ME'}
              </button>
            </motion.div>
